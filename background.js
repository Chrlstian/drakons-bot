...
function executeScript() {
    if (!isRunning) return; // Stop if the script is flagged as stopped
    ...
    function waitForImageAndClick() {
        function clickImage() {
            if (!isRunning) return; // Stop execution if the stop button was clicked
            const divs = document.querySelectorAll('._3h-UINLQJpiyToPQuDbXiL._3biUgzO4Xw_UFNpbhRQ7_x');
            if (divs.length >= 4) {
                const fourthDivImage = divs[3].querySelector('.ui.tiny.image');
                if (fourthDivImage) {
...
// Start or stop the script based on message received
...
chrome.alarms.onAlarm.addListener((alarm) => {
    if (alarm.name === 'drakonsAlarm' && isRunning) {
        executeScript();
    }
});
